...
$(function(){
    ...
    $("#btn").click(function(){
        ...
        if(fileType!='zip'){
            layer.msg('只能上传zip为后缀的文件哦',{time:2000});
            return false;
        }
        //限制文件上传大小
        if(fileSize>maxSize){
            layer.msg('文件大小不能超过100M，请重新选择',{time:2000});
            $("#uploadFile").val('');
            return false;
        }
        ...
        vertifyVersion(formObj.identify,formObj.name,formObj.build,function(data){
            if(data.canUpload){
                $.ajax({
                    ...
                    success:function(dataBack){
                        if(dataBack.isExpired){
                            layer.msg(dataBack.msg,{time:2000},function(){
                ...
                layer.msg(data.msg+",当前服务器上的最新版本是:"+data.currentVersion);
            }
        });
